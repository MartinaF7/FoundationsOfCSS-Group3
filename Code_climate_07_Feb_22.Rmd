---
title: "Foundations of CSS group project - The impact of emotional language on engagement rates on Twitter - Climate change"
author: "Group 3"
---
...
```{r}
df_climate = search_tweets(q = "climate change", n = 15000, type = "mixed", include_rts = F, lang = "en")
```

```{r}
climate_filtered = df_climate %>%
...
```{r}
users_vector_climate = climate_filtered[['user_id']]
temp_df_users_climate <- lookup_users(users = users_vector_climate)
```

```{r}
followers_filtered_climate = temp_df_users_climate %>%
                               select(user_id, followers_count) 
                         
climate_final = inner_join(climate_filtered, followers_filtered_climate)
```

```{r}
climate_final$engagement_rate <- (climate_final$retweet_count/climate_final$followers_count)*100
```
...
```{r}
twitterVADER <- vader_df(climate_final$text)
```

```{r}
climate_final$VADERcompound <- twitterVADER$compound
```

```{r}
climate_final$syuzhet_value <- get_sentiment(climate_final$text)
```

```{r}
climate_final <- na.omit(climate_final) 
```
...
```{r}
mean(climate_final$engagement_rate, na.rm = TRUE)
sd(climate_final$engagement_rate, na.rm = TRUE)

#calculating 95% confidence interval
ci <- qnorm(0.975)*sd(climate_final$engagement_rate, na.rm = TRUE)/sqrt(nrow(climate_final))
mean(climate_final$engagement_rate, na.rm = TRUE)-ci
mean(climate_final$engagement_rate, na.rm = TRUE)+ci
```
...
```{r}
hist(climate_final$VADERcompound, main="VADER sentiment scores of tweets regarding climate change", xlab="Sentiment of tweets (VADER score)", ylab ="Frequency")
abline(v = mean(climate_final$VADERcompound, na.rm = TRUE), col="red", lwd=3, lty=1)

hist(climate_final$syuzhet_value, main="Syuzhet sentiment scores of tweets regarding climate change", xlab="Sentiment of tweets (Syuzhet score)", ylab ="Frequency")
abline(v = mean(climate_final$syuzhet_value, na.rm = TRUE), col="red", lwd=3, lty=1)
```

Relationship between VADER compound/ Syuzhet values and engagement rates: 

```{r}
filtered = climate_final %>%
             filter(engagement_rate < mean(climate_final$engagement_rate, na.rm = TRUE)+ci)
...
```{r}
plot(filtered$syuzhet_value, filtered$engagement_rate, main="Engagement rate on Twitter vs Syuzhet sentiment scores \n of tweets regarding climate change", xlab="Sentiment of tweets (Syuzhet score)", ylab="Engagement rate (%)")

plot(filtered$VADERcompound, filtered$engagement_rate, main="Engagement rate on Twitter vs VADER sentiment scores \n of tweets regarding climate change", xlab="Sentiment of tweets (VADER score)", ylab="Engagement rate (%)")
```

```{r}
# LM model - Vader

lm_model_V = lm(VADERcompound~engagement_rate, data=climate_final)
summary(lm_model_V)
...
lm_model_S = lm(syuzhet_value~engagement_rate, data=climate_final)
summary(lm_model_S)
...
```{r}
VADER_positive = climate_final %>%
                   filter(VADERcompound >= 0.5) %>%
                     filter(engagement_rate != 'Inf')

VADER_negative = climate_final %>%
                   filter(VADERcompound <= -0.5) %>%
                     filter(engagement_rate != 'Inf')

VADER_neutral = climate_final %>%
                  filter(VADERcompound < 0.5) %>%
                    filter(VADERcompound > -0.5) %>%
                      filter(engagement_rate != 'Inf')
...
```{r}
syuzhet_positive = climate_final %>%
                     filter(syuzhet_value >= 2.5) %>%
                       filter(engagement_rate != 'Inf')

syuzhet_negative = climate_final %>%
                     filter(syuzhet_value <= -2.5) %>%
                       filter(engagement_rate != 'Inf')

syuzhet_neutral = climate_final %>%
                    filter(syuzhet_value < 2.5) %>%
                      filter(syuzhet_value > -2.5) %>%
                        filter(engagement_rate != 'Inf')
...
Open a dataframe climate_final and arrange the rows by engagement_rate so that the highest engagement rates are at the top. For each of the top 5 tweets with highest engagement rates, at least the following analysis should be carried out:
...
```{r}
# For each of the 5 tweets with highest engagement rates, look up the user and the tweet separately. From the climate_final dataframe, copy-paste the user ID and status ID for each of the 5 top tweets into the code below. Run this code 5 times for the 5 tweets. 
...
Also take a look at dataframe climate_final as a whole and note down any interesting observations e.g. if there are some tweets with high engagement rates but low number of followers and vice versa. Consider the same factors as above - for example, could it be that the time when the tweet was posted and the time we retrieved the data is close in proximity so that is why it did not receive more retweets?   
