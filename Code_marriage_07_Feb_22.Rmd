---
title: "Foundations of CSS group project - The impact of emotional language on engagement rates on Twitter - Same Sex Marriage"
author: "Group 3"
---
...
```{r}
df_marriage = search_tweets(q = "same sex marriage", n = 15000, type = "mixed", include_rts = F, lang = "en")
```

```{r}
marriage_filtered = df_marriage %>%
...
```{r}
users_vector_marriage = marriage_filtered[['user_id']]
temp_df_users_marriage <- lookup_users(users = users_vector_marriage)
```

```{r}
followers_filtered_marriage = temp_df_users_marriage %>%
                               select(user_id, followers_count) 
                         
marriage_final = inner_join(marriage_filtered, followers_filtered_marriage)
```

```{r}
marriage_final$engagement_rate <- (marriage_final$retweet_count/marriage_final$followers_count)*100
```
...
```{r}
twitterVADER <- vader_df(marriage_final$text)
```

```{r}
marriage_final$VADERcompound <- twitterVADER$compound
```

```{r}
marriage_final$syuzhet_value <- get_sentiment(marriage_final$text)
```

```{r}
marriage_final <- na.omit(marriage_final) 
```
...
```{r}
mean(marriage_final$engagement_rate, na.rm = TRUE)
sd(marriage_final$engagement_rate, na.rm = TRUE)

#calculating 95% confidence interval
ci <- qnorm(0.975)*sd(marriage_final$engagement_rate, na.rm = TRUE)/sqrt(nrow(marriage_final))
mean(marriage_final$engagement_rate, na.rm = TRUE)-ci
mean(marriage_final$engagement_rate, na.rm = TRUE)+ci
```
...
```{r}
hist(marriage_final$VADERcompound, main="VADER sentiment scores of tweets regarding same sex marriage", xlab="Sentiment of tweets (VADER score)", ylab ="Frequency")
abline(v = mean(marriage_final$VADERcompound, na.rm = TRUE), col="red", lwd=3, lty=1)

hist(marriage_final$syuzhet_value, main="Syuzhet sentiment scores of tweets regarding same sex marriage", xlab="Sentiment of tweets (Syuzhet score)", ylab ="Frequency")
abline(v = mean(marriage_final$syuzhet_value, na.rm = TRUE), col="red", lwd=3, lty=1)
```

Relationship between VADER compound/ Syuzhet values and engagement rates: 

```{r}
filtered = marriage_final %>%
             filter(engagement_rate < mean(marriage_final$engagement_rate, na.rm = TRUE)+ci)
...
```{r}
plot(filtered$syuzhet_value, filtered$engagement_rate, main="Engagement rate on Twitter vs Syuzhet sentiment scores \n of tweets regarding same sex marriage", xlab="Sentiment of tweets (Syuzhet score)", ylab="Engagement rate (%)")

plot(filtered$VADERcompound, filtered$engagement_rate, main="Engagement rate on Twitter vs VADER sentiment scores \n of tweets regarding same sex marriage", xlab="Sentiment of tweets (VADER score)", ylab="Engagement rate (%)")
```

```{r}
# LM model - Vader

lm_model_V = lm(VADERcompound~engagement_rate, data=marriage_final)
summary(lm_model_V)
...
lm_model_S = lm(syuzhet_value~engagement_rate, data=marriage_final)
summary(lm_model_S)
...
```{r}
VADER_positive = marriage_final %>%
                   filter(VADERcompound >= 0.5) %>%
                     filter(engagement_rate != 'Inf')

VADER_negative = marriage_final %>%
                   filter(VADERcompound <= -0.5) %>%
                     filter(engagement_rate != 'Inf')

VADER_neutral = marriage_final %>%
                  filter(VADERcompound < 0.5) %>%
                    filter(VADERcompound > -0.5) %>%
                      filter(engagement_rate != 'Inf')
...
ggplot(VADERdf, aes(x=VADER_Sentiment_Score_Category, y=Mean_Engagement_Rate)) + geom_errorbar(aes(ymin=Mean_Engagement_Rate-Confidence_Interval, ymax=Mean_Engagement_Rate+Confidence_Interval), width=.2) +  geom_line() + geom_point() + ggtitle("Mean engagement rates for positive, neutral and negative sentiment Tweets regarding \nsame sex marriage classified on the basis of their VADER sentiment scores") + xlab("VADER Sentiment Score Category") + ylab("Mean Engagement Rate (%) and 95% CI")
```

```{r}
# Check the distribution of engagement rate data for normality to determine which test to carry out.

hist(VADER_positive$engagement_rate, main="Engagement rates of Tweets with positive VADER scores \n regarding same sex marriage", xlab="Engagement rate (%)", ylab ="Frequency")
...
```{r}
syuzhet_positive = marriage_final %>%
                     filter(syuzhet_value >= 2.5) %>%
                       filter(engagement_rate != 'Inf')

syuzhet_negative = marriage_final %>%
                     filter(syuzhet_value <= -2.5) %>%
                       filter(engagement_rate != 'Inf')

syuzhet_neutral = marriage_final %>%
                    filter(syuzhet_value < 2.5) %>%
                      filter(syuzhet_value > -2.5) %>%
                        filter(engagement_rate != 'Inf')
...
ggplot(syuzhetdf, aes(x=Syuzhet_Sentiment_Score_Category, y=Mean_Engagement_Rate)) + geom_errorbar(aes(ymin=Mean_Engagement_Rate-Confidence_Interval, ymax=Mean_Engagement_Rate+Confidence_Interval), width=.2) +  geom_line() + geom_point() + ggtitle("Mean engagement rates for positive, neutral and negative sentiment Tweets regarding \nsame sex marriage classified on the basis of their Syuzhet sentiment scores") + xlab("Syuzhet Sentiment Score Category") + ylab("Mean Engagement Rate (%) and 95% CI")
```
...
Open a dataframe marriage_final and arrange the rows by engagement_rate so that the highest engagement rates are at the top. For each of the top 5 tweets with highest engagement rates, at least the following analysis should be carried out:
...
```{r}
# For each of the 5 tweets with highest engagement rates, look up the user and the tweet separately. From the marriage_final dataframe, copy-paste the user ID and status ID for each of the 5 top tweets into the code below. Run this code 5 times for the 5 tweets. 
...
Also take a look at dataframe marriage_final as a whole and note down any interesting observations e.g. if there are some tweets with high engagement rates but low number of followers and vice versa. Consider the same factors as above - for example, could it be that the time when the tweet was posted and the time we retrieved the data is close in proximity so that is why it did not receive more retweets?   
